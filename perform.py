    ...
    with st.spinner("多軌平行量化引擎運算中..."):
        ...
        try:
            ...
            if not STOCKS_POOL:
                ...
                # 🏁 多軌交叉新標的戰略部署 (篩選核心變更為：三軌天數下都合格的真正核心新標的)
                ...
                def format_stocks(df):
                    ...
                    return "\n".join([f"* {row['股票名稱']} `(乖離:{row['50MA乖離率(%)']:.1f}%)` [20日:{row['20日抗跌分']:.0f}分 | 30日:{row['30日抗跌分']:.0f}分 | 45日:{row['45日抗跌分']:.0f}分]" for _, row in df.iterrows()])
