        ...
        try:
            if not STOCKS_POOL:
                ...
                if b_c.empty:
                    ...
                    def format_stocks(df, is_backtesting_mode, show_perf=False):
                        if df.empty:
                            return "無"
                        lines = []
                        for _, row in df.iterrows():
                            ...
                            # 🆕 處置股狀態標記 (回測模式下不顯示)
                            disp_str = ""
                            if not is_backtesting_mode:
                                disp_info = DISPOSITION_MAP.get(row['股票代號'])
                                if disp_info:
                                    period_text = f" ({disp_info['period']})" if disp_info.get('period') else ""
                                    # 加了 <br> 強制換行
                                    disp_str = (
                                        f"<br><span style='background-color: #fff1f0; color: #e74c3c; border: 1px solid #ffbaba; "
                                        f"padding: 2px 6px; border-radius: 6px; font-size: 0.85em; font-weight: 600; "
                                        f"margin-left: 0px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);'>"
                                        f"🚨 處置中{period_text}</span>"
                                    )
                            
                            line = f"- {row['股票名稱']} ({bias_str}%){disp_str}{perf_str}"
                            lines.append(line)
                        return "\n".join(lines)
                    ...
                    with col1:
                        st.subheader("🌟 市場領頭羊 (強勢+抗跌)")
                        st.markdown(format_stocks(true_leaders, is_backtesting, show_perf=is_backtesting), unsafe_allow_html=True)
                        st.subheader("⚠️ 短線動能強勢 (相對抗跌不足)")
                        st.markdown(format_stocks(momentum_only, is_backtesting, show_perf=is_backtesting), unsafe_allow_html=True)
                    
                    with col2:
                        st.subheader("🛡️ 防守型避風港 (弱勢+抗跌)")
                        st.markdown(format_stocks(defensive_only, is_backtesting, show_perf=is_backtesting), unsafe_allow_html=True)
                        st.subheader("📉 落後補跌區")
                        st.markdown(format_stocks(laggards, is_backtesting, show_perf=is_backtesting), unsafe_allow_html=True)
