                ...
                if b_c.empty:
                    ...
                    def format_stocks(df, show_perf=False, is_backtesting=False):
                        ...
                        for _, row in df.iterrows():
                            ...
                            # 🆕 處置股狀態標記 (置於 50MA乖離率 之後，並判斷非回測才顯示)
                    ...
                    with col1:
                        st.markdown(f"**📈 強勢領導股 (Leading):**\n\n{format_stocks(true_leaders, show_perf=True, is_backtesting=is_backtesting)}")
                        st.markdown(f"**🚀 動能爆發股 (Momentum):**\n\n{format_stocks(momentum_only, show_perf=True, is_backtesting=is_backtesting)}")
                    with col2:
                        st.markdown(f"**🛡️ 防守抗跌股 (Defensive):**\n\n{format_stocks(defensive_only, show_perf=True, is_backtesting=is_backtesting)}")
                        st.markdown(f"**📉 落後補漲/弱勢股 (Laggards):**\n\n{format_stocks(laggards, show_perf=True, is_backtesting=is_backtesting)}")
