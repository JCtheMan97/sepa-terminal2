...
# --- 🚀 個人持股風控與健檢輔助函數 ---
def format_ticker_smart(ticker):
    """
    智慧後綴偵測：利用 STOCK_DICT 自動比對上市.TW或上櫃.TWO
    """
    t = str(ticker).strip()
    if t == 'REALIZED_CASH':
        return t
    ...
    if match:
        ...
        if f"{pure_code}.TWO" in STOCK_DICT:
            return f"{pure_code}.TWO"
        return f"{pure_code}.TW"
    return t

def calculate_portfolio_signals(ticker, buy_date_str, avg_cost, current_price, hist_df):
    try:
        if hist_df.empty or len(hist_df) < 222:
            return {"action": "⏳ 數據不足", "color": "blue", "checklist": "歷史 K 線不足 222 天，無法計算完整 MA"}

        close_series = hist_df['Close']
        volume_series = hist_df['Volume']
        ...
        # 修正：精準補齊 MA200 扣抵 22 日前的趨勢
        sma200_22 = close_series.rolling(200).mean().iloc[-23] if len(close_series) >= 223 else np.nan
        
        # 判斷趨勢樣板 (Trend Template)
        is_p1 = (current_price > sma150) and (current_price > sma200)
        is_p2 = sma150 > sma200
        is_p3 = (sma200 > sma200_22) if not pd.isna(sma200_22) else False
        is_p4 = (sma50 > sma150) and (sma50 > sma200)
        is_p5 = (current_price > sma50)
        
        highest_252 = close_series.tail(252).max()
        lowest_252 = close_series.tail(252).min()
        is_p6 = ((current_price / lowest_252) - 1) >= 0.25
        is_p7 = (1 - (current_price / highest_252)) <= 0.25
        
        is_trend_template = is_p1 and is_p2 and is_p3 and is_p4 and is_p5 and is_p6 and is_p7

        # 找出買入後最高收盤價
        buy_datetime = pd.to_datetime(buy_date_str)
        post_buy_df = hist_df[hist_df.index >= buy_datetime]
        max_close_since_buy = post_buy_df['Close'].max() if not post_buy_df.empty else current_price
            
        profit_p = ((current_price - avg_cost) / avg_cost) * 100
        
        # 🛡️ 保本邏輯：獲利曾達 10% 後移至成本
        has_hit_be_target = max_close_since_buy >= (avg_cost * 1.10)
        sl_threshold = -10.0
        dynamic_sl_price = avg_cost if has_hit_be_target else (avg_cost * (1 + sl_threshold / 100))
        
        # 50MA 乖離率
        bias_50 = ((current_price - sma50) / sma50) * 100
        is_extended = bias_50 > 25.0
        
        # 量能
        sma20_vol = volume_series.rolling(20).mean().iloc[-1]
        is_climax_vol = volume_series.iloc[-1] > (sma20_vol * 3.0)
        
        # 雙軌背離判定
        is_price_new_high = current_price >= close_series.tail(30).max()
        is_div_warning = is_price_new_high and (current_price < sma5)

        action_ref = "⏳ 觀察等待"
        action_color = "blue"
        checklist = []
        
        if current_price <= dynamic_sl_price:
            if has_hit_be_target:
                action_ref = "🛡️ 保本離場"
                action_color = "green"
                checklist.append("❌ 獲利曾達10%且跌回成本線")
            else:
                action_ref = "✂️ 立即停損"
                action_color = "red"
                checklist.append(f"❌ 虧損已達停損限制 ({profit_p:.1f}%)")
        elif profit_p > 20 and current_price < sma20:
            action_ref = "🚨 獲利回吐警告"
            action_color = "orange"
            checklist.append("⚠️ 獲利 >20% 且收盤跌破 20MA")
        elif profit_p > 0 and is_div_warning:
            action_ref = "💰 考慮分批獲利"
            action_color = "orange"
            checklist.append("⚠️ 雙軌背離且收破 5MA")
        elif profit_p > 0 and is_extended:
            if is_climax_vol:
                action_ref = "🌋 拋售高潮減碼"
                action_color = "red"
                checklist.append(f"⚠️ 50MA極端乖離 ({bias_50:.1f}%) 且爆3倍量")
            else:
                action_ref = "🚀 拋售高潮預警"
                action_color = "red"
                checklist.append(f"⚠️ 50MA極端乖離 ({bias_50:.1f}%)")
        elif profit_p > 0:
            action_ref = "💎 強勢續抱"
            action_color = "green"
            checklist.append(f"✅ 獲利中 (+{profit_p:.1f}%)")
            if is_trend_template:
                checklist.append("🔥 符合馬克第二階段樣板")
        else:
            action_ref = "⏳ 輕度套牢觀察"
            action_color = "blue"
            checklist.append(f"📉 套牢中 ({profit_p:.1f}%)，停損防守位: {dynamic_sl_price:.2f}")

        return {
            "action": action_ref,
            "color": action_color,
            "checklist": " | ".join(checklist),
            "dynamic_sl": dynamic_sl_price,
            "has_hit_be": has_hit_be_target,
            "profit_p": profit_p,
            "max_close": max_close_since_buy,
            "current_price": current_price
        }
    except Exception as e:
        return {"action": "⚠️ 計算錯誤", "color": "gray", "checklist": str(e)}

def crawl_mops_news_openapi(tickers_nums):
    """
    使用 OpenAPI 下載今日重大訊息並本地過濾
    """
    news_results = []
    if not tickers_nums:
        return news_results
        
    tickers_set = set(str(t).strip() for t in tickers_nums)
    headers = {"User-Agent": "Mozilla/5.0"}
    
    # 1. TWSE
    try:
        url_twse = "https://openapi.twse.com.tw/v1/opendata/t187ap04_L"
        r = requests.get(url_twse, headers=headers, verify=False, timeout=8)
        if r.status_code == 200:
            for item in r.json():
                code = str(item.get("公司代號", item.get("Code", ""))).strip()
                if code in tickers_set:
                    date_str = str(item.get("發言日期", item.get("Date", ""))).strip()
                    title = str(item.get("主旨", item.get("Subject", ""))).strip()
                    news_results.append({
                        "代碼": code,
                        "發言日期": date_str,
                        "主旨摘要": title
    ...
    # 2. TPEx
    try:
        url_tpex = "https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap04_O"
        r = requests.get(url_tpex, headers=headers, verify=False, timeout=8)
        if r.status_code == 200:
            for item in r.json():
                code = str(item.get("SecuritiesCompanyCode", item.get("Code", ""))).strip()
                if code in tickers_set:
                    date_str = str(item.get("Date", item.get("發言日期", ""))).strip()
                    title = str(item.get("主旨", item.get("Subject", ""))).strip()
                    if not title or title == "None":
                        for k, v in item.items():
                            if k not in ["Date", "SecuritiesCompanyCode", "CompanyName"] and len(str(v)) > 20:
                                title = str(v)
                                break
                    news_results.append({
                        "代碼": code,
...
# --- 🎯 運行模式選擇 ---
st.sidebar.markdown("### 🖥️ 系統運作模式")
run_mode = st.sidebar.radio(
    "請選擇終端機功能：",
    ["🔍 SEPA 選股掃描模式", "📊 個人持股風控模式", "📡 即時重大訊息預警模式"]
)
st.sidebar.markdown("---")
...
# 📊 個人持股風控模式 UI 與運算
# ==========================================
if run_mode == "📊 個人持股風控模式":
    st.subheader("📊 馬克·米納爾維尼動態保本與部位風控矩陣")
    
    if uploaded_file is None:
        st.info("💡 請在左側安全面板上傳您的 `portfolio.csv`，或點擊下方按鈕載入示範資料體驗。")
        if st.button("🚀 使用 14 檔示範持股載入演示"):
            example_data = """Ticker,Buy_Date,Avg_Cost,Shares
2337,2026-05-19,149.5,3000
3028,2026-07-02,82.6,6000
6187,2026-02-03,373.5,500
3037,2025-11-07,161.5,500
3017,2026-02-11,191.29,170
8086,2026-05-06,156.0,3000
4749,2026-05-15,104.22,450
3680,2026-04-28,485.0,1000
8021,2026-03-25,248.5,1000
3481,2026-05-06,28.1,10000
8438,2026-04-07,78.0,5000
3691,2026-05-11,144.5,3000
2423,2026-06-17,76.55,6000
3305,2026-06-05,152.33,3000"""
            import io
            uploaded_file = io.StringIO(example_data)

    if uploaded_file is not None:
        portfolio_df = pd.read_csv(uploaded_file)
        portfolio_df = portfolio_df[portfolio_df['Ticker'] != 'REALIZED_CASH']
        portfolio_df['Ticker_Formatted'] = portfolio_df['Ticker'].apply(format_ticker_smart)
        
        active_tickers = portfolio_df['Ticker_Formatted'].tolist()
        
        with st.spinner("安全載入即時價格數據並計算 SEPA 部位指標..."):
            results_list = []
            end_date = datetime.today().date()
            start_date = end_date - timedelta(days=400) # 確保足夠天數算 200MA
            
            tickers_str = " ".join(active_tickers)
            try:
                # 批次下載價格
                df_all = yf.download(tickers_str, start=start_date.strftime('%Y-%m-%d'), end=(end_date + timedelta(days=2)).strftime('%Y-%m-%d'), progress=False)
                ...
                st.error(f"yfinance 數據載入失敗: {e}")
                df_adj = None
                df_vol = None
                
            for _, row in portfolio_df.iterrows():
                ticker = row['Ticker_Formatted']
                buy_date = row['Buy_Date']
                avg_cost = float(row['Avg_Cost'])
                shares = int(row['Shares'])
                
                if df_adj is not None and ticker in df_adj.columns:
                    s_series = df_adj[ticker].dropna()
                    s_vol = df_vol[ticker].dropna().reindex(s_series.index).ffill()
                    
                    ticker_hist = pd.DataFrame({'Close': s_series, 'Volume': s_vol})
                    
                    if not ticker_hist.empty:
                        current_price = float(s_series.iloc[-1])
                        m_results = calculate_portfolio_signals(ticker, buy_date, avg_cost, current_price, ticker_hist)
                        m_results['shares'] = shares
                        m_results['ticker'] = ticker.split(".")[0]
                        m_results['buy_date'] = buy_date
                        m_results['avg_cost'] = avg_cost
                        results_list.append(m_results)
                    
            results_df = pd.DataFrame(results_list)

        if not results_df.empty:
            col1, col2, col3, col4 = st.columns(4)
            total_market_value = sum(results_df['current_price'] * results_df['shares'])
            total_cost = sum(results_df['avg_cost'] * results_df['shares'])
            total_profit = total_market_value - total_cost
            total_profit_p = (total_profit / total_cost) * 100 if total_cost > 0 else 0
            
            with col1:
                st.metric("📦 總持股市值", f"${total_market_value:,.0f} 元")
            with col2:
                st.metric("💰 未實現損益", f"${total_profit:,.0f} 元", f"{total_profit_p:.2f}%")
            with col3:
                danger_counts = len(results_df[results_df['action'].isin(["✂️ 立即停損", "🛡️ 保本離場", "🚨 獲利回吐警告"])])
                st.metric("🚨 風險警戒個股", f"{danger_counts} 檔", delta="- 需處置" if danger_counts > 0 else "安全", delta_color="inverse")
            with col4:
                be_active_counts = len(results_df[results_df['has_hit_be'] == True])
                st.metric("🛡️ 已啟動保本個股", f"{be_active_counts} 檔", "安全鎖定")

            st.write("---")
            st.write("### 📋 持股風控矩陣詳細報告")
            
            display_cols = ['ticker', 'buy_date', 'avg_cost', 'current_price', 'profit_p', 'dynamic_sl', 'action', 'checklist']
            styled_df = results_df[display_cols].copy()
            styled_df.columns = ['股票代碼', '買入日期', '買入成本', '最新價', '目前報酬 (%)', '動態停損位', '大師決策指令', '觸發核心條件']
            ...
            def highlight_actions(val):
                ...
                elif val in ["🚨 獲利回吐警告", "💰 考慮分批獲利", "🌋 拋售高潮減碼"]:
                    return 'background-color: #7c2d12; color: white;'
                elif val == "💎 強勢續抱":
                    return 'background-color: #0f172a; color: #4ade80; font-weight: bold;'
                return 'background-color: #0f172a; color: #38bdf8;'

            st.dataframe(
                styled_df.style.applymap(highlight_actions, subset=['大師決策指令'])
                .format({'買入成本': '{:.2f}', '最新價': '{:.2f}', '目前報酬 (%)': '{:+.2f}%', '動態停損位': '{:.2f}'}),
                use_container_width=True, hide_index=True
            )
            
            # --- 🏦 擔保品維持率診斷 ---
            st.write("---")
            st.subheader("🏦 融資與質押擔保品安全邊際診斷")
            st.caption("輸入您的信用交易關鍵數據，結合即時收盤變動，自動計算動態維持率與健檢報告。")
            
            c1, c2 = st.columns(2)
            with c1:
                prev_margin_ratio = st.number_input("昨日對帳單「維持率」 (%)", minval=100.0, maxval=500.0, value=166.0, step=1.0)
                loan_balance = st.number_input("目前融資/質押貸款總餘額 (元)", minval=0, value=2000000, step=10000)
            with c2:
                call_threshold = st.number_input("追繳維持率門檻 (%)", value=130.0, step=1.0)
                
            st.write("### 📎 勾選哪些持股作為此次質押/融資之擔保品：")
            selected_collateral = st.multiselect("選擇質押標的", options=[f"{t}.TW" if f"{t}.TW" in active_tickers else f"{t}.TWO" for t in results_df['ticker']], default=[f"{t}.TW" if f"{t}.TW" in active_tickers else f"{t}.TWO" for t in results_df['ticker']][:min(5, len(results_df))])
            
            collateral_mv = 0
            for _, r in results_df[results_df['ticker'].isin([t.split(".")[0] for t in selected_collateral])].iterrows():
                collateral_mv += r['current_price'] * r['shares']
                
            est_margin_ratio = (collateral_mv / loan_balance) * 100 if loan_balance > 0 else 0
                
            st.write("#### 🛡️ 擔保品健康狀態指標")
            mc1, mc2, mc3 = st.columns(3)
            with mc1:
                st.metric("💼 當前擔保品總市值", f"${collateral_mv:,.0f} 元")
            with mc2:
                st.metric(
                    "📈 盤中動態預估維持率", 
                    f"{est_margin_ratio:.1f}%", 
                    delta=f"較昨日: {est_margin_ratio - prev_margin_ratio:+.1f}%"
                )
            with mc3:
                if est_margin_ratio >= 140:
                    st.success("🟢 狀態安全 (高於維持水位 140%)")
                elif est_margin_ratio >= 130:
                    st.warning("🟡 警訊：逼近追繳線 130%！")
                else:
                    st.error("🔴 嚴重警報：低於 130%！面臨追繳斷頭風險！")
...
# 📡 即時重大訊息預警模式 UI
# ==========================================
if run_mode == "📡 即時重大訊息預警模式":
    st.subheader("📡 即時重大訊息預警衛星 (官方直連版)")
    
    if uploaded_file is None:
        st.info("💡 請在左側安全面板上傳您的 `portfolio.csv`，或點擊下方按鈕載入示範持股代碼同步重訊。")
        if st.button("🚀 載入示範持股代碼演示"):
            example_data = """Ticker,Buy_Date,Avg_Cost,Shares
2337,2026-05-19,149.5,3000
3028,2026-07-02,82.6,6000
6187,2026-02-03,373.5,500
...
3680,2026-04-28,485.0,1000"""
    ...
    if uploaded_file is not None:
        ...
        active_tickers = portfolio_df['Ticker_Formatted'].tolist()
        pure_ticker_nums = [str(t).split(".")[0] for t in active_tickers]
        
        st.info(f"📡 當前監控標的代碼: {', '.join(pure_ticker_nums)}")
        
        if 'mops_news_cache' not in st.session_state:
            st.session_state.mops_news_cache = []
            
        if st.button("🚀 開始/同步最新重大訊息"):
            with st.spinner("同步證交所與櫃買中心今日重大訊息中..."):
                news = crawl_mops_news_openapi(pure_ticker_nums)
                st.session_state.mops_news_cache = news
                if news:
                    st.success(f"⚡ 同步完成！共獲取到 {len(news)} 筆符合的重大訊息。")
                else:
                    st.info("ℹ️ 今日目前未偵測到持股之相關重大訊息公告。")
                    
        if st.session_state.mops_news_cache:
            news_df = pd.DataFrame(st.session_state.mops_news_cache)
            news_df.columns = ["代碼", "發言日期", "主旨摘要"]
...
# --- 🛡️ 安全與風控面板 (持股上傳) ---
st.sidebar.markdown("### 🛡️ 安全與風控面板")
uploaded_file = st.sidebar.file_uploader(
    "📂 上傳您的投資組合 CSV (portfolio.csv)",
    type=["csv"],
    help="CSV 需包含欄位: Ticker, Buy_Date, Avg_Cost, Shares"
)
st.sidebar.caption("🔒 隱私安全：資產數據僅暫存於瀏覽器中。")
st.sidebar.markdown("---")
